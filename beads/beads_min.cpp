...
const string USAGE = "usage: beads_min width file min_beads [dumpfile]\n";
...
int main(int argc, char * argv[])
{
    ...
    if (pixels == NULL)
    {
        cout << "TIFF didn't open correctly\n";
        cout << "Exiting due to undetermined error.\n";
        return -1;
    }

    // number of beads. There are three estimates/entries per bead plus three for S0, B0, and theta0
    beads = 0;
    size_t p = 3;                                                                                                                                                               // size t is an unsigned int normally used for indexing arrays
    ...
    */
    ...
    while ((IC < oldIC) || (beads < min_beads));
